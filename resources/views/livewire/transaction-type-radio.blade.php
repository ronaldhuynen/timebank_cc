<div class="mt-4 max-w-md" x-data>
    ...
    @if (in_array('work', $typeOptions))
        <x-radio id="transTypeRadio-1" label="{{ __('Worked time: for the total time worked or helped') }}" value="work"
                 wire:model.live="transTypeRadio" />
    @endif
    @if (in_array('gift', $typeOptions))
        <x-radio id="transTypeRadio-2" label="{{ __('Gift: without something in return') }}" value="gift"
                 wire:model.live="transTypeRadio" />
    @endif
    @if (in_array('donation', $typeOptions))
        <x-radio id="transTypeRadio-3" label="{{ __('Donation: to support the cause of this organization') }}"
