    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex justify-between items-center h-16">
            ...
            <div class="hidden space-x-8 items-center sm:-my-px sm:ml-10 sm:flex">
                ...
                <x-jetstream.nav-link href="{{ route('user.edit') }}" :active="request()->routeIs('user.edit')">
                    {{ __('Commons') }}
                </x-jetstream.nav-link>
                @can('manage posts')
                <x-jetstream.nav-link href="{{ route('posts.manage') }}" :active="request()->routeIs('posts.manage')">
                    {{ __('Posts') }}
                </x-jetstream.nav-link>
                @endcan
                <!-- Main Search Bar -->
        ...
        <div class="hidden sm:flex sm:items-center sm:ml-6">
                ...
                <x-jetstream.dropdown align="right" width="60">
                    ...
                    <x-slot name="content">
                        <div class="w-60">
                            ...
                            <!-- Team Settings -->
                            <x-jetstream.dropdown-link href="{{ route('teams.show', Auth::user()->currentTeam->id) }}">
                                {{ __('Team Settings') }}
                ...
                <x-jetstream.dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        <livewire:select-organization>
                            ...
                            <x-jetstream.dropdown-link href="{{ route('user.edit') }}" :active="request()->routeIs('user.edit')">
                                {{ __('Edit Profile') }}
                            </x-jetstream.dropdown-link>
                            @elseif (session('activeProfileType') == 'App\Models\Organization')
                            <x-jetstream.dropdown-link href="{{ route('org.edit') }}" :active="request()->routeIs('org.show')">
                                {{ __('Edit profile') }}
                            </x-jetstream.dropdown-link>
                            @endif
            ...
            <form method="POST" action="{{ route('logout') }}" x-data>
                @csrf
                <x-jetstream.dropdown-link href="{{ route('logout') }}" @click.prevent="$root.submit();">
                    {{ __('Log Out') }}
                </x-jetstream.dropdown-link>
            </form>
            </x-slot>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <x-jetstream.dropdown-link href="{{ route('profile.show') }}">
                ...
                <form method="POST" action="{{ route('logout') }}" x-data>
                    ...
                    <x-jetstream.responsive-nav-link href="{{ route('logout') }}" @click.prevent="$root.submit();">
                        {{ __('Log Out') }}
                    </x-jetstream.responsive-nav-link>
                </form>
                ...
                <!-- Team Settings -->
                <x-jetstream.responsive-nav-link href="{{ route('teams.show', Auth::user()->currentTeam->id) }}" :active="request()->routeIs('teams.show')">
                    {{ __('Team Settings') }}
                </x-jetstream.responsive-nav-link>

                @can('create', Laravel\Jetstream\Jetstream::newTeamModel())
                <x-jetstream.responsive-nav-link href="{{ route('teams.create') }}" :active="request()->routeIs('teams.create')">
                    {{ __('Create New Team') }}
                </x-jetstream.responsive-nav-link>
                @endcan
