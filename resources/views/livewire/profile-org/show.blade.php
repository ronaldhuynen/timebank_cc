         ...
         <section>
             ...
             <div class="relative rounded-lg border-gray-600 bg-black border-1 px-12 py-16 shadow-xl dark:border-gray-700">
                 ...
                 <div class="mt-3 flex justify-between">
                     @if ($isOnline)
                     <img class="h-60 w-60 flex-shrink-0 rounded-full object-cover ring-4 ring-green-400 sm:mx-4" src="{{ Storage::url($org->profile_photo_path) }}" alt="{{ $org->name }}">
                     @elseif ($isAway)
                     <img class="h-60 w-60 flex-shrink-0 rounded-full object-cover ring-4 ring-sky-400 sm:mx-4" src="{{ Storage::url($org->profile_photo_path) }}" alt="{{ $org->name }}">
                     @else
                     <img class="h-60 w-60 flex-shrink-0 rounded-full object-cover ring-4 ring-gray-300 sm:mx-4" src="{{ Storage::url($org->profile_photo_path) }}" alt="{{ $org->name }}">
                     @endif
                     <div class="pl-4 sm:mx-4 sm:mt-0">
                         ...
                         <div class="flex justify-between">
                             ...
                             <span class="flex justify-between">
                                 ...
                                 <span class="flex justify-between">
                                     ...
                                     <!--- Like button -->
                                     @livewire('like-button', ['model' => $org, 'likedCounter' => $org->likedCounter, 'typeName' => 'Star'])
                         ...
                         <div class="mt-4 grid grid-cols-3 gap-1 text-gray-400 dark:text-gray-200 md:grid-cols-3 xl:grid-cols-3">
                             ...
                             @if (session('activeProfileType') === $org::class && session('activeProfileId') !== $org->id || session('activeProfileType') !== $this->org::class)

                             <div class="">
                                 @if (count($friend) < 1 && count($pendingFriend) < 1) <!-- Not a friend -->
                                     <a wire:click="friendRequest" class="text-lg font-bold text-gray-400 underline hover:text-gray-100 dark:text-gray-500" onclick="FriendsManager.action({dropdown : true, provider_id : '{{$org->id}}', action : 'add', provider_alias : 'user'}); 
                                            return false;" href="#"><i class="fas fa-user-plus"></i><br />
                                         {{ __('Friend request') }}</a>
                                     @elseif (count($pendingFriend) > 0 )
                                     <!-- Pending friend -->
                                     <a wire:click="cancelFriendRequest" class="text-lg font-bold text-gray-400 underline hover:text-gray-100 dark:text-gray-500" onclick="FriendsManager.action({dropdown : true, provider_id : '{{$org->id}}', action : 'cancel', sent_friend_id : '{{ $pendingFriend->first()->id }}', provider_alias : 'user'}); return false;" href="#"><i class="fas fa-ban"></i>
                                         <br />
                                         {{ __('Cancel friend request') }}</a>
                                     @else
                                     <!-- Is a friend -->
                                     <a wire:click="cancelFriendRequest" class="text-lg font-bold text-gray-400 underline hover:text-gray-100 dark:text-gray-500" onclick="FriendsManager.action({dropdown : true, provider_id : '{{$org->id}}', action : 'remove', friend_id : '{{ $friend->first()->id }}', provider_alias : 'user'}); return false;" href="#"><i class="fas fa-user-times"></i>
                                         <br />
                 ...
                 <div x-data="{ showAboutFullText: false }">
                     <p class="mt-4 text-gray-300 group-hover:text-gray-100 dark:text-gray-300">
                         <span x-show="!showAboutFullText">
                             {{ Illuminate\Support\Str::limit($org->about ?? '', 300) }}
                         </span>
                         <span x-show="showAboutFullText">
                             {{ $org->about ?? '' }}
                         </span>
